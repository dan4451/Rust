...
namespace Oxide.Plugins
{
    ...
    public class TowCars : CovalencePlugin
    {
        ...
        private void CmdTow(IPlayer p, string cmd, string[] args)
        {
            ...
            if (active.TryGetValue(carA.net.ID.Value, out var link))
            {
            ...
            active[carA.net.ID.Value] = newLink;
            cooldowns[bp.userID] = Time.realtimeSinceStartup + config.Cooldown;
            LogDebug($"Tow attached: {carA.net.ID.Value} -> {carB.net.ID.Value} by {bp.UserIDString}");
        ...
        {
            return bp.GetMountedVehicle() as BaseEntity; // Returns the vehicle entity directly
        }
        BaseEntity FindCarInFront(BasePlayer bp, float maxDist)
        {
            ...
            RaycastHit[] hits = Physics.BoxCastAll(
                ...
                maxDist,
                LayerMask.GetMask("Vehicle", "Deployed")
            );
        ...
        bool IsInNoTowZone(BaseEntity ent)
        {
            ...
            return (player != null && player.InSafeZone()) || ent.GetComponentInParent<MonumentInfo>() != null;
        }
        bool TryAttach(BaseEntity carA, BaseEntity carB, out TowLink link, out string reason)
        {
            link = null; reason = null;
            var rbA = carA?.GetComponent<Rigidbody>();
            var rbB = carB?.GetComponent<Rigidbody>();
            ...
            if (active.ContainsKey(carB.net.ID.Value)) { reason = "Target is already towing something."; return false; }
        ...
        void Detach(TowLink link, bool notify = false)
        {
            ...
            var rb = link.A?.GetComponent<Rigidbody>();
            if (rb != null) rb.drag = link.Drag;
            active.Remove(link.A.net.ID.Value);
            LogDebug($"Tow detached: {link.A?.net.ID.Value} -> {link.B?.net.ID.Value}");
        ...
        void OnEntityKill(BaseNetworkable ent)
        {
            ...
            // If towing car dies
            TowLink link;
            if (active.TryGetValue(be.net.ID.Value, out link)) Detach(link);
